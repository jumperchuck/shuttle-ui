...
const GridSectionList = <T, SectionT = DefaultSectionT>(
  ...
  const layoutManager = useMemo(() => {
    if (lm) {
      lm.row = row;
      lm.col = col;
      lm.cellWidth = cellWidth;
      lm.cellHeight = cellHeight;
      lm.pageWidth = pageWidth;
      lm.pageHeight = pageHeight;
      lm.spacing = spacing;
      lm.horizontal = !!horizontal;
      lm.initialNumToRender = 0;
      lm.renderItem = fnRef.current.renderItem;
      lm.renderNoItem = fnRef.current.renderNoItem;
      lm.getItemLayout = fnRef.current.getItemLayout ?? (() => ({ row: 1, col: 1 }));
      lm.getSectionHeaderLayout = fnRef.current.getSectionHeaderLayout;
      lm.getSectionFooterLayout = fnRef.current.getSectionFooterLayout;
      lm.nodeSections = [];
      lm.indexLayouts = [];
    }
    const manager =
...
GridSectionList.useLayoutManager = useLayoutManager;
